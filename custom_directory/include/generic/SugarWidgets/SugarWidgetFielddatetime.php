...
/**
 * MMR Customization.
 * The only code changed here is in displayListWeek
 */
/*
...
class SugarWidgetFieldDateTime extends SugarWidgetReportField
{
    ...
    {
        // MMR Customization - Start
        $match = array();
        if (preg_match('/(\d{4})-(\d+)/', $this->displayListPlain($layout_def), $match)) {
            $startDate = (new DateTime())->setISODate($match[1], $match[2])->format('Y-m-d');
            return 'W' . $match[2] . ': ' . $startDate;
        }
